(() => {
  ...
  function makeId() {
    if (globalThis.crypto?.randomUUID) return `lr-${crypto.randomUUID()}`;
  ...
  function showToast(message) {
    ...
    shadow.innerHTML = `
      <style>
        .toast{font:600 12px/1.4 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#f8fafc;background:#0f172a;border:1px solid rgba(255,255,255,.12);box-shadow:0 10px 30px rgba(15,23,42,.24);border-radius:999px;padding:8px 12px;letter-spacing:.01em}
      </style>
  ...
  async function saveMemo(memo) {
    ...
    await chrome.storage.local.set({ [`${ENTRY_PREFIX}${id}`]: entry });
    showToast(`ログを保存 ${formatJstTime(now)}`);
    return true;
  }
  ...
  function openCapture() {
    ...
    const shadow = host.attachShadow({ mode: 'open' });
    const currentTime = formatJstTime();
    ...
    shadow.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}
        ...
        .head{display:flex;align-items:center;justify-content:space-between;margin:0 2px 8px;color:#64748b;font-size:11px;font-weight:700;letter-spacing:.02em}
        .name{color:#334155}
      ...
      <div class="relay" role="dialog" aria-label="Log Relayにメモを追加">
        <div class="head"><span class="name">↪ Log Relay</span><span>${escapeHtml(currentTime)}</span></div>
        <div class="row">
          <input type="text" maxlength="500" autocomplete="off" placeholder="いま残しておくこと…" aria-label="ログメモ">
    ...
    const submit = async () => {
      ...
      try {
        const saved = await saveMemo(input.value);
        if (saved) closeCapture();
      } catch (error) {
